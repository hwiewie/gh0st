...
BOOL CKeyBoardDlg::OnInitDialog() 
{
	...
	if (pSysMenu != NULL)
	{
		...
		pSysMenu->AppendMenu(MF_STRING, IDM_ENABLE_OFFLINE, "離線記錄(&O)");
		pSysMenu->AppendMenu(MF_STRING, IDM_CLEAR_RECORD, "清空記錄(&C)");
		pSysMenu->AppendMenu(MF_STRING, IDM_SAVE_RECORD, "保存記錄(&S)");
	...
	m_edit.SetLimitText(MAXDWORD); // 設置最大長度
	ResizeEdit();
	UpdateTitle();
...
bool CKeyBoardDlg::SaveRecord()
{
	...
	CFileDialog dlg(FALSE, "txt", strFileName, OFN_OVERWRITEPROMPT, "文本文檔(*.txt)|*.txt|", this);
	if(dlg.DoModal () != IDOK)
		return false;
