...
void CMainFrame::Activate(UINT nPort, UINT nMaxConnections)
{
	...
	// 開啟IPCP服務器
 	...
 	{
 		str.Format("端口%d綁定失敗", nPort);
 		m_wndStatusBar.SetPaneText(0, str);
 		m_wndStatusBar.SetPaneText(2, "端口: 0");
 	}

	m_wndStatusBar.SetPaneText(3, "連接: 0");
}
...
void CMainFrame::OnSysCommand(UINT nID, LPARAM lParam)
{
	if (nID == SC_MINIMIZE)
	{
		if (!m_TrayIcon.Enabled())
			...
			AfxGetApp()->LoadIcon(IDR_MAINFRAME), IDR_MINIMIZE, TRUE); //構造
		ShowWindow(SW_HIDE);
	}
	else
	{
...
{
	CString str;
	str.Format("連接: %d", g_pConnectView->GetListCtrl().GetItemCount());
