...
void CSettingsView::OnActivateView(BOOL bActivate, CView* pActivateView, CView* pDeactiveView) 
{
	...
	if (m_bFirstShow)
	{
		...
		int i = 0;
		for (; i < sizeof(nEditControl) / sizeof(int); i++)
		...
		for (i = 0; i < sizeof(nBtnControl) / sizeof(int); i++)
		{
 		...
 		if (((CGh0stApp *)AfxGetApp())->m_bIsQQwryExist)
 			SetDlgItemText(IDC_SYSTEM_TIPS, "找到IP數據庫文件: QQWry.Dat");
 		else
 			SetDlgItemText(IDC_SYSTEM_TIPS, "沒有找到IP數據庫文件QQWry.Dat 請將此文件放至本程序同目錄下以顯示服務端地理位置");
	}
...
fail:
	...
	if (nRet == SOCKET_ERROR)
		AfxMessageBox("無法連接到代理服務器");
	else if (nRet == 0)
		AfxMessageBox("用戶名/密碼錯誤");
	else if (nRet == 1)	
		AfxMessageBox("代理服務器工作正常");
	return -1;
}
...
fail:
	...
	if (bRet)
		strResult.Format("成功打開到主機%s的連接， 在端口 %s: 連接成功", pThis->m_remote_host, pThis->m_remote_port);
	else
		strResult.Format("不能打開到主機%s的連接， 在端口 %s: 連接失敗", pThis->m_remote_host, pThis->m_remote_port);
