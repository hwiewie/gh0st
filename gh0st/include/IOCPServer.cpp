...
CIOCPServer::CIOCPServer()
{
	...
	m_nKeepLiveTime = 1000 * 60 * 3; // 三分鐘探測一次
	// Packet Flag;
...
void CIOCPServer::OnAccept()
{
   ...
   // Associate the new socket with a completion port.
	...
	// 關閉nagle算法,以免影響性能，因為控制時控制端要發送很多數據量很小的數據包,要求馬上發送
...
// 	}
	...
	klive.keepaliveinterval = 1000 * 10; // 重試間隔為10秒 Resend if No-Reply
	WSAIoctl
		(
		pContext->m_Socket, 
		SIO_KEEPALIVE_VALS,
...
void CIOCPServer::Send(ClientContext* pContext, LPBYTE lpData, UINT nSize)
{
	...
	try
	{
		if (nSize > 0)
		{
			...
			// 發送完後，再備份數據, 因為有可能是m_ResendWriteBuffer本身在發送,所以不直接寫入
			LPBYTE lpResendWriteBuffer = new BYTE[nSize];
			CopyMemory(lpResendWriteBuffer, lpData, nSize);
			pContext->m_ResendWriteBuffer.ClearBuffer();
			pContext->m_ResendWriteBuffer.Write(lpResendWriteBuffer, nSize);	// 備份發送的數據
		...
		else // 要求重發
		{
			pContext->m_WriteBuffer.Write(m_bPacketFlag, sizeof(m_bPacketFlag));
			pContext->m_ResendWriteBuffer.ClearBuffer();
			pContext->m_ResendWriteBuffer.Write(m_bPacketFlag, sizeof(m_bPacketFlag));	// 備份發送的數據	
		}
