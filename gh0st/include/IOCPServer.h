...
struct ClientContext
{
	...
	CBuffer				m_CompressionBuffer;	// 接收到的壓縮的數據
	CBuffer				m_DeCompressionBuffer;	// 解壓後的數據
	CBuffer				m_ResendWriteBuffer;	// 上次發送的數據包，接收失敗時重發時用

	int					m_Dialog[2]; // 放對話框列表用，第一個int是類型，第二個是CDialog的地址
...
public:
	...
	UINT					m_nSendKbps; // 發送即時速度
	UINT					m_nRecvKbps; // 接受即時速度
	UINT					m_nMaxConnections; // 最大連接數
protected:
